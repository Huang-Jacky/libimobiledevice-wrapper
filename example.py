...
def sync_example():
    ...
    try:
        ...
        device.get_device_logs(udid, log_file)
...
def app_logs_example():
    ...
    try:
        ...
        # 获取应用日志（30秒，包含错误关键字）
        print("\n获取应用日志（30秒，过滤错误信息）...")
        logs = device.get_app_logs(udid, bundle_id, duration=30, keywords=['error', 'Error', 'exception'])
        ...
        device.get_app_logs(udid, bundle_id, duration=10, output_file=log_file)
...
def device_logs_example():
    ...
    try:
        ...
        with device.capture_device_logs(
            udid=udid, 
            ...
            import time
            time.sleep(5)  # 监控5秒
            print(f"已捕获 {len(capture.get_logs())} 条日志")
...
def cli_commands_example():
    ...
    print("4. 日志监控:")
    print("   libidevice device-logs --udid <udid> --keywords '[MTG' --output logs.txt")
